...
<?php
    $email = $rows['Doc_Email'];
    if(isset($_POST['verify_btn'])){
        $query = "UPDATE doc_account set D_Valid='active'";
        $result = mysqli_query($db , $query); 
        if($result){
            $subject = "Account Verified";
            $body = "Your Account is Verified you can now login...";
    ...
    if(isset($_POST['delete_btn'])){
        ...
        if($result){
            ...
            $body = "Your Account has been deleted because some information maybe incorrect";
            $headers = "From:connectus1111@gmail.com";
            if(mail($email, $subject, $body, $headers)){	
                header("location:verifydoc.php");
            }
